...
\end{figure}
Martin Fowler, on the other hand, warned against taking on too much deliberate debt. He argues that `[e]ven the best teams will have debt to deal with as a project goes on - even more reason not to overload it with crummy code.'\footcite{fowlerTechnicalDebtQuadrant2009}
...
This finding emphasizes the important of proactively managing the software quality and addressing debt early in the project life cycle, to keep the complexity and therefore the cost to a minimum.\\
...
infrastructure or requirements debt, makes these tools useful but not a complete solution. A general evaluation of the default rules as well as inspecting if the results should all be necessarily be implemented is therefore important when working with tools like SonarQube.\\
...
On the code level, a variety of practices can be used to prevent software decay and technical debt. The two major practices have been previously introduced: continuous refactoring and maintaining software quality through various practices like CI/CD or a high test coverage.\\
...
technical debt enables targeted interventions before significant decay occurs. Ultimately, strategic decisions on refactoring versus comprehensive redesign, which encompasses a full re-architecture of the system to accommodate changed requirements, should be 
...
The second distinction is the different perspectives of the actors involved. Unlike in \ac{COTS}, where it is usually only the project manager 
...
These projects often have a life cycle of decades compared to the commercial software life cycle of a few years.\footcite[14]{shyu2017military}
Due to these long life cycles, the software needs to be developed so it can be easily maintained and updated, while still working with legacy systems
...
Like all federal agencies in Germany, the Bundeswehr must adhere to the \ac{BSI} guidelines for IT security, called IT-Grundschutz\footcite{BSIFAQ}. This defines a comprehensive security framework
...
This leads to an increase in the complexity and effort required for ongoing software maintenance and updates.\footcite{BSIStandards}\\
...
As Sebastian Sprenger reported in 2018 for Defense News, the `digitization of the head-up-display likely will require the costly replacement of the aircraft’s main computer, officials wrote.'\footcite{sprengerGermanyFearsSteeper2018} This upgrade was a NATO mandatory requirement, indicating how the outdated system architecture and hardware make new mandatory requirements difficult to implement. 
